...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <?php
        $basicData   = json_decode($application['form_data'], true) ?: [];
        $basicFields = ['nombre' => 'Nombre', 'apellidos' => 'Apellidos', 'email' => 'Email', 'telefono' => 'Telefono'];
        // If basic fields are missing from form_data (overwritten after form submission),
        // reconstruct from applicant_name stored at creation time
        if (empty($basicData['nombre']) && !empty($application['applicant_name'])) {
            $nameParts = explode(' ', $application['applicant_name'], 2);
            $basicData['nombre']    = $nameParts[0] ?? $application['applicant_name'];
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <?php elseif ($formLinkStatus === 'enviado' && !empty($formLinkId)): ?>
                <?php $clientFormUrl = BASE_URL . '/public/form/' . intval($formLinkId) . '?app=' . $application['id']; ?>
